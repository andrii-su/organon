...
pub fn scan(root: &str, graph: Arc<Mutex<Graph>>, ignore_set: &IgnoreSet) -> Result<ScanStats> {
    info!("scanning: {}", root);
        ...
        match Entity::from_path(&path_str) {
            Ok(entity) => {
                match graph.lock().unwrap().upsert(&entity) {
                    Ok(_) => { indexed.fetch_add(1, std::sync::atomic::Ordering::Relaxed); }
                    Err(e) => {
...
pub fn refresh_lifecycle(graph: Arc<Mutex<Graph>>, lc: &LifecycleConfig) -> Result<usize> {
    ...
    for mut entity in entities {
        let new_state = compute_state(entity.accessed_at, now, lc.dormant_days, lc.archive_days);
